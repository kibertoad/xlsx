...
|------------------------|------------------------------------------------------------------------------------------------------------|
| \`@office-kit/xlsx/io\`          | \`loadWorkbook\`, \`saveWorkbook\`, \`workbookToBytes\`; browser-safe \`fromArrayBuffer\` / \`fromBlob\` / \`fromResponse\` / \`fromStream\` / \`toArrayBuffer\` / \`toBlob\`; types \`XlsxSource\`, \`XlsxSink\`, \`LoadOptions\`, \`SaveOptions\` (\`mtime\` pins the per-entry ZIP timestamp for byte-identical output, recorded in UTC, year 1980-2099; \`compressionLevel\` 0-9, and \`CompressionLevel\` from \`@office-kit/xlsx/zip\` names the union) |
...
Both default to the used range, so a sheet formatted past its content yields
rows that are entirely \`null\`. Bound the walk by the value extent to skip
them:

\`\`\`ts
import { getValueExtent, iterValues } from '@office-kit/xlsx/worksheet';

const box = getValueExtent(ws); // undefined when no cell holds a value
if (box) {
  for (const row of iterValues(ws, box)) console.log(row);
}
\`\`\`
...
| Find the used range (formatting included) | \`getCellExtent\` (or \`getMaxRow\` + \`getMaxCol\`) |
| Find where the values end | \`getValueExtent\` |
| Iterate only the rows holding values | \`getValueExtent\` + \`iterValues(ws, box)\` |
