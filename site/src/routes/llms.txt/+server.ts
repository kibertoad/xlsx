...
| \`@office-kit/xlsx/utils\`       | A1 ↔ row/col conversion (\`coordinateFromString\`, \`coordinateToTuple\`, \`tupleToCoordinate\`, \`columnLetterFromIndex\`, \`columnIndexFromLetter\`, \`rangeBoundaries\`, \`boundariesToRangeString\`, \`parseSheetRange\`; \`tupleToCoordinate\` / \`boundariesToRangeString\` take \`{ absoluteCol, absoluteRow }\` for the \`$B$5\` / \`$A$4:$H$20\` forms, and the \`RangeRef\` union (\`string | { minRow, minCol, maxRow, maxCol }\`) every range-taking helper accepts, \`formatSheetQualifiedRef\`, validators \`isValidCellRef\` / \`isValidRangeRef\` / \`isValidColumnLetter\` / \`isValidColumnNumber\` / \`isValidRowNumber\`); Excel date helpers (\`excelToDate\`, \`dateToExcel\`, \`excelToDuration\`, \`durationToExcel\`, \`toIso8601\`, \`fromIso8601\`); EMU/pixel/point unit conversion; \`escapeCellString\` / \`unescapeCellString\`; \`inferCellType\` + \`ERROR_CODES\`; error hierarchy \`OpenXmlError\`, \`OpenXmlIoError\`, \`OpenXmlSchemaError\`, \`OpenXmlDecompressionBombError\`, \`OpenXmlInvalidWorkbookError\`, \`OpenXmlNotImplementedError\` |
...
| Build \`$B$5\` / \`$A$4:$H$20\` from numbers | \`tupleToCoordinate\` / \`boundariesToRangeString\` with \`{ absoluteCol: true, absoluteRow: true }\` |
| Address a range from integer bounds | any range helper with \`{ minRow, minCol, maxRow, maxCol }\` |
