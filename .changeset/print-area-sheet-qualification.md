...
`setPrintArea` also throws `OpenXmlSchemaError` when `sheetIndex` names no sheet on the workbook, instead of registering a name scoped past the end of the sheet list, and when `ref` is empty or has an empty leg (`A1:B2,,D1:E2`).
